...
void main()
{
	...
	//-- appel de fonction --// 
	longeurSegment = CalculerDistance_A_B(ptA_x, ptA_y, ptB_x, ptB_y);	// Declaration de calcule de la distance
	...
	ptM_x = CalculerMilieuAxe_X(ptA_x, ptB_x);	// Point 2
	...
	//-> longeur du Segment
	printf("\n longeur du segment : %f", longeurSegment);
	
	//-> point milieu du segement 
	printf("\n point milieu du segement A-B est : [ %f ; %f ] ", ptM_x);
